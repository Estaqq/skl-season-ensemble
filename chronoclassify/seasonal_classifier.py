...
class SeasonalClassifier(ClassifierMixin, BaseEstimator):
    """An classifier, which manages a collection of base estimators of type base_model_class and 
    trains and calls them depending on the value of the feature of the name or index passed 
    in time_column.
    ...
    window_start : int, float, default=None
        The lower bound for the range for which we expect data. If None, 
        the minimum value occuring in the training data is used.
    window_end : int, float, default=None
        The upper bound for the range for which we expect data. If None, 
        the maximum value occuring in the training data is used.
    n_windows : int, default=10
        The number of windows in which the data is split. If window_size is set, 
        this parameter is ignored.
    windows : list, default=None
        A list of the boundaries of the windows. If set, 
        window_size, window_start and window_end are ignored.
    padding : int, float, default=105
        The distance in time units away from some window for which data is 
        still considered for training the classifier for that particular window.
    time_column : str, int, default=0
        The index or name of the column in the input data that contains the 
        time information based on which data is assigned to base classifiers.
        Strings can only be passed, if the input data is a pandas DataFrame or Series.
        When using SeasonalClassifier together with functions such as grid_search or 
        cross_validate, either a numerical index must be provided,
        or the argument col_names must also be provided, since those functions pass 
        numpy arrays during their invocation of SeasonalClassifier.
    ...
    drop_time_column : bool, default=True
        Whether to drop the time column from the input data before passing it to 
        the base classifiers. Ignored when the input data has only one feature.
    data_is_periodic : bool, default=True
        Whether the data is periodic. If True, data is considered to be periodic and the windows are 
        wrapped around the window_start and window_end values.
    ...
    def _create_models(self):
        ...
        self._models = []
        assert len(self._internal_windows) > 1
        for _ in range( len(self._internal_windows) -1 ):
    ...
    def _fit_base_models(self):
        for i, model in enumerate(self._models):
            ...
            if self._drop_time_column :
                model.fit(np.delete(self.X_[selection,:], self._time_column,axis = 1), self.y_[selection])
            else:
                model.fit(self.X_[selection,:], self.y_[selection])
            if self.verbose:
    ...
    def fit(self, X, y):
        ...
        if isinstance(self.time_column, str):
    ...
    def predict_proba(self, X):
        """Predict the probabilities of the input samples belonging to each class using the 
    ...
    def predict_log_proba(self, X):
        """Predict the log-probabilities of the input samples belonging to each class 
        using the appropriate base model.
